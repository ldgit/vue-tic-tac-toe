...
describe('Ultimate.playSquare', () => {
  ...
  ['0', 0].forEach((squareIndex) => {
    test(
      `playing a square on initial board should advance to next player (square${JSON.stringify(squareIndex)})`,
      () => {
    ...
    test(
      `playing a square on initial board should mark that square as played by current player (square ${JSON.stringify(squareIndex)})`,
      () => {
        const newState = Ultimate.playSquare(initialState, { boardIndex: 0, squareIndex });
  ...
  test(
    'changing board squares of new state should not alter previous state (immutability)',
    () => {
  ...
  test('second move should correctly update next player', () => {
    const newState = Ultimate.playSquare(initialState, { boardIndex: 0, squareIndex: 0 });
    const finalState = Ultimate.playSquare(newState, { boardIndex: 0, squareIndex: 4 });
    assert.equal(finalState.nextPlayer, 'X');
  });

  test('second move should mark that square as played by first player', () => {
    const newState = Ultimate.playSquare(initialState, { boardIndex: 0, squareIndex: 0 });
    const finalState = Ultimate.playSquare(newState, { boardIndex: 0, squareIndex: 4 });
  ...
  [0, 1, 3, 8, '2', '7'].forEach((squareIndex) => {
    // eslint-disable-next-line max-len
    test(
      `a move marks all boards except one that must be played next as inactive (squareIndex: ${JSON.stringify(squareIndex)})`,
      () => {
        const newState = Ultimate.playSquare(initialState, { boardIndex: 0, squareIndex });
  ...
  // eslint-disable-next-line max-len
  test(
    'move played on inactive board square does nothing (next player is sent to board in relative location to played square) *',
    () => {
      const newState = Ultimate.playSquare(initialState, { boardIndex: 0, squareIndex: 0 });
  ...
  [
    ...
    test(`move played on won board does nothing (${description}) *`, () => {
  ...
  test('playing the same square twice will not advance to next player', () => {
  ...
  test(
    'playing the same square twice will not change the value of that square',
    () => {
  ...
  [
    ...
    test(`new moves do nothing if game won ${description} *`, () => {
  ...
  [4, '4'].forEach((squareIndex) => {
    // eslint-disable-next-line max-len
    test(
      `clicking on an empty square on inactive board does not change boards isActive status (${JSON.stringify(squareIndex)})`,
      () => {
  ...
  test('playing an occupied square on inactive board does nothing', () => {
  ...
  // eslint-disable-next-line max-len
  test(
    'if move takes the player to won board, mark all boards as active (covers the case where move that takes the player to won board is also the one that won that board)',
    () => {
  ...
  test(
    'playing a move on inactive board that takes the player to a won board should do nothing',
    () => {
      const firstXMoveState = Ultimate.playSquare(initialState, { boardIndex: 0, squareIndex: 8 });
      const firstOMoveState = Ultimate.playSquare(firstXMoveState, { boardIndex: 8, squareIndex: 0 });
      const secondXMoveState = Ultimate.playSquare(firstOMoveState, { boardIndex: 0, squareIndex: 4 });
  ...
  describe('history and time travel', () => {
    test('valid move should push new board state to history', () => {
    ...
    test('valid move should increment pointInHistory in state', () => {
      let currentState;
    ...
    test('invalid move should not add more entries to history', () => {
    ...
    test('changing one history entry should not change any others', () => {});

    describe('jump to a point in history', () => {
      test('should change pointInHistory property', () => {
        const newState = Ultimate.playSquare(initialState, { boardIndex: 1, squareIndex: 8 });
      ...
      test('should change next player accordingly', () => {
      ...
      test('should return a deep copy of input state', () => {
        const newState = Ultimate.playSquare(initialState, { boardIndex: 1, squareIndex: 8 });
      ...
      test(
        'when time traveling to past, all rules related to that move apply',
        () => {
          const moveOneState = Ultimate.playSquare(initialState, { boardIndex: 4, squareIndex: 0 });
          const moveTwoState = Ultimate.playSquare(moveOneState, { boardIndex: 0, squareIndex: 2 });
          const moveThreeState = Ultimate.playSquare(moveTwoState, { boardIndex: 2, squareIndex: 3 });
      ...
      test(
        'Play one move, then back to game start, then repeat the move. Next player should be O.',
        () => {
          const moveOneState = Ultimate.playSquare(initialState, { boardIndex: 4, squareIndex: 0 });
      ...
      test('should not delete history entries', () => {
      ...
      test(
        'after playing a new valid move, all history after that move is discarded',
        () => {
          const midState = Ultimate.playSquare(initialState, { boardIndex: 1, squareIndex: '8' });
      ...
      test(
        'playing an invalid move, history is unchanged (already tested in tests marked with *)',
        () => {},
      );
    });
  });
...
describe('deepCopyGameState', () => {
  test('should copy top level properties', () => {
  ...
  test('should copy squares for each boards array in history', () => {
    const initialState = Ultimate.getInitialState();
